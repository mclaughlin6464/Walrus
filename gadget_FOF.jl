...
type particle
    x::Array{Float32,2} # positions
    v::Array{Float32,2} # velocities
...
filename = "/Users/tabel/Research/OldResearch/TetraPart/data/23eV/Gadget/NP128/snapshot_039"
rcut = 5
Nbins = 15

h = read_gadget_header(filename)
p = read_gadget_data(filename)

Npart = length(p.id)

gr = zeros(p.id) # array to hold the group number
l = 0.1 * h.BoxSize/Npart^.33334 # linking length in gadget units

balltree = BallTree(p.x) # Build tree
count = 0
ids = []
for i in 1:Npart
    idxs = inrange(balltree, p.x[:,i], l, true)
    Np = length(idxs)
    Np > 1 ? count += 1 : nothing
end
println(count)
